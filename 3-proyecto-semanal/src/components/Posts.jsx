...
function Posts({posts}){
    ...
    return(
        <section className="flex w-full gap-6">
            <Link key={postGrande._id} to={`/post/${postGrande.slug}`} className="flex flex-5 flex-col gap-5"> 
            ...
            <div className="flex-5 flex flex-col gap-4">
                {postsChicos.map((post) => ( //el return esta implicito
                    <Link key={post._id} to={`/post/${post.slug}`}  className="flex-1 flex gap-3 h-full">
